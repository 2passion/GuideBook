...
- 콘텐츠 파일 변경 금지.
- .gitignore 정책 외의 Git 설정은 변경하지 않는다.
  (branch / remote / tag 변경 금지)
...
3. commit & push
   .gitignore 변경과 git rm --cached 결과를 함께 커밋한다.
...
5. git status 최종 확인
6. 특이사항
7. 다음 commit부터 archive 백업 파일이 자동 제외되는지 확인
   (generator.py 실행 후 git status에서 archive 파일 미표시 여부)
